...
\starthead {\bf ID utilisé pour la note}~: Les notes sont toujours identifiées par un numéro. Ce que nous pouvons configurer ici est~:\stophead
  ...
  \item {\em Numérotation avec symboles}~: on peut utiliser quatre jeux de symboles différents selon l'option choisie~: {\tt set~0, set~1, set~2} ou {\tt set~3}. Sur \at{page} [exemples de jeux de conversion], vous trouverez un exemple des symboles utilisés dans chacune de ces options.
