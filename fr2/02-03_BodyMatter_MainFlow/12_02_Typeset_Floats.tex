...
Malgré ce qui vient d'être dit, si la langue principale du document n'est pas l'anglais, le nom anglais des objets prédéfinis, comme par exemple les objets \MyKey{figure} ou \MyKey{table}, sera traduit~; Ainsi, par exemple, l'objet \MyKey{figure} dans les documents en français est appelé \MyKey{Figure}, tandis que l'objet \MyKey{table} est appelé \MyKey{Table}. Ces noms français pour les objets prédéfinis peuvent être modifiés avec \tex{setuplabeltext} comme expliqué dans \in{section}[sec:labels].
