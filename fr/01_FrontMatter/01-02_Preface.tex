...
\startchapter[title={Table des matières},
...
\definemeasure [xA] [21.0mm]
\definemeasure [yA] [29.7mm]

\definemeasure [xA] [25.2mm]
\definemeasure [yA] [35.64mm]

\definemeasure [xm] [10.0mm]
\definemeasure [ym] [10.0mm]

\definemeasure [xB] [\dimexpr2\measured{xA}+1\measured{xm}\relax]
\definemeasure [xC] [\dimexpr4\measured{xA}+3\measured{xm}\relax]
\definemeasure [xD] [\dimexpr5\measured{xA}+4\measured{xm}\relax]

\definemeasure [yE] [\dimexpr4\measured{yA}+3\measured{ym}\relax]
...
%fill (unitsquare xyscaled (w,h)) withcolor \MPcolor{white} ;

% =============================================================================

pair pbase ; pbase := (1.5cm,28.0cm); 

pair ppasxA ; ppasxA := (21.0mm, 0.0mm); 
pair ppasyA ; ppasyA := ( 0.0cm,29.7mm);

ppasxA := (25.2mm, 0.0mm); 
ppasyA := ( 0.0cm,35.64mm);

pair ppasx ; ppasx := ppasxA + (10mm, 0.0mm); 
pair ppasy ; ppasy := ppasyA + ( 0mm,10.0mm); 

draw textext.lrt("\BigCase{\sc \bf I}")
     shifted (pbase + 0 * ppasx - 0 * ppasy);

draw textext.lrt("\Case[backgroundcolor=white]{\tt TXT}")
     shifted (pbase + 1 * ppasx - 0 * ppasy);

draw textext.lrt("\Case[width=\measure{xB}]{%
     {\bf \ConTeXt}\blank[small]
     {\bf\in{}{ - Panorama}[cap:panorama]}\blank[small]
     {\bf\in{}{ - Premier doc}[cap:firstdoc]}\blank[small]
     {\bf\in{}{ - Commandes}[cap:commands]}\blank[small]
     {\bf\in{}{ - Fichier source}[cap:sourcefile]}}")
     shifted (pbase + 2 * ppasx - 0 * ppasy);

draw textext.lrt("\Case[backgroundcolor=white]{\tt PDF}")
     shifted (pbase + 4 * ppasx - 0 * ppasy);


%------------------------------------------------------------------------------

draw textext.lrt("\BigCase[height=\measure{yE}]{\sc\bf II}")
     shifted (pbase + 0 * ppasx - 1 * ppasy);

% draw textext.urt("\externalfigure[frame.pdf]
%      [width=2.1cm,frame=on,page=2]")
%      shifted (pbase - 1 * ppasy);

draw textext.lrt("\Case[width=\measure{xB}]{%
     {\bf\in{}{ - Pagination}[cap:pages]}\\
     \in{}{ - Taille de la page}         [sec:papersize]\\
     \in{}{ - Mise en page}              [sec:pagelayout]\\
     \in{}{ - Numérotation pages}        [sec:numpages]\\
     \in{}{ - En-têtes et pieds de page} [sec:headerfooter]\\
     \in{}{ - Marges}                    [sec:margintext]\\
%     \in{}{ - Sauts de page}             [sec:pagebreak]
}")
     shifted (pbase + 1 * ppasx - 1 * ppasy);

draw textext.lrt("\externalfigure[frame.pdf]
     [width=\measure{xA},frame=on,page=1]")
     shifted (pbase + 3 * ppasx - 1 * ppasy);

draw textext.lrt("\Case{%
     {\bf\in{}{ - Polices\\ de caractère \\ et couleurs} [cap:fontscol]}}")
     shifted (pbase + 4 * ppasx - 1 * ppasy);

%------------------------------------------------------------------------------

draw textext.lrt("\Case[height=11mm,width=\measure{xC}]{%
     {\bf\in{}{ - Structure du document} [cap:structure]} \\
         \in{}{ - Macro-structure}[sec:macrostructure]}")
     shifted (pbase + 1 * ppasx - 2 * ppasy);

draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]
     [width=\measure{xA},frame=on,page=1,
      background=color,backgroundcolor=white]")
     shifted (pbase + 0 * ppasx - 2 * ppasy - (0mm,13mm));

draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]
     [width=\measure{xA},frame=on,page=5,
      background=color,backgroundcolor=white]")
     shifted (pbase + 1 * ppasx - 2 * ppasy - (0mm,13mm));

draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]
     [width=\measure{xA},frame=on,page=11,
      background=color,backgroundcolor=white]")
     shifted (pbase + 2 * ppasx - 2 * ppasy - (0mm,13mm));

draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]  %17 pour 14
     [width=\measure{xA},frame=on,page=19,
      background=color,backgroundcolor=white]")
     shifted (pbase + 3 * ppasx - 2 * ppasy - (0mm,13mm));
     
draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]
     [width=\measure{xA},frame=on,page=21,
      background=color,backgroundcolor=white]")
     shifted (pbase + 4 * ppasx - 2 * ppasy - (0mm,13mm));




draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]
     [width=\measure{xA},frame=on,page=107,
      background=color,backgroundcolor=white]")
     shifted (pbase + 0 * ppasx - 3 * ppasy - (0mm,5mm));

draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]
     [width=\measure{xA},frame=on,page=235,
      background=color,backgroundcolor=white]")
     shifted (pbase + 1 * ppasx - 3 * ppasy - (0mm,5mm));
     
draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]
     [width=\measure{xA},frame=on,page=261,
      background=color,backgroundcolor=white]")
     shifted (pbase + 2 * ppasx - 3 * ppasy - (0mm,5mm));
     
draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]
     [width=\measure{xA},frame=on,page=263,
      background=color,backgroundcolor=white]")
     shifted (pbase + 3 * ppasx - 3 * ppasy - (0mm,5mm));
     
draw textext.lrt("\externalfigure[introCTX_fra_for_TOC.pdf]
     [width=\measure{xA},frame=on,page=274,
      background=color,backgroundcolor=white]")
     shifted (pbase + 4 * ppasx - 3 * ppasy - (0mm,5mm));

%------------------------------------------------------------------------------


draw textext.lrt("\Case{%
     {\bf\in{}{ - Table \\ des matières, \\ index, listes} [cap:toc]}}")
     shifted (pbase + 1 * ppasx - 4 * ppasy);

draw textext.lrt("\Case{%
     {\bf\in{}{ - Références et hyperliens} [cap:refpluslinks]}}")
     shifted (pbase + 2 * ppasx - 4 * ppasy);

%------------------------------------------------------------------------------

draw textext.lrt("\BigCase{\sc \bf III}")
     shifted (pbase + 0 * ppasx - 5 * ppasy);

draw textext.lrt("\Case{%
{\bf\in{}{ - Lettres, \\ mots et \\ espaces \\ horizontaux}     [cap:chartext]}}")
     shifted (pbase + 1 * ppasx - 5 * ppasy);

draw textext.lrt("\Case{%
{\bf\in{}{ - \\ Paragraphes, \\ lignes et \\ espaces \\ verticaux} [cap:parlinevspace]}}")
     shifted (pbase + 2 * ppasx - 5 * ppasy);

draw textext.lrt("\Case{%
{\bf\in{}{ - \\ Constructions \\ spéciales et \\ paragraphes}      [cap:specialparas]}}")
     shifted (pbase + 3 * ppasx - 5 * ppasy);

draw textext.lrt("\Case{%
{\bf\in{}{ - Images, \\ tableaux et \\ autres \\ flottants} [cap:floats]}}")
     shifted (pbase + 4 * ppasx - 5 * ppasy);

%------------------------------------------------------------------------------

drawarrow (pbase + 1 * ppasx + 1 * ppasxA - 0.5 * ppasyA) -- 
          (pbase + 2 * ppasx              - 0.5 * ppasyA)
          withpen pencircle scaled 2bp;

drawarrow (pbase + 3 * ppasx + 1 * ppasxA - 0.5 * ppasyA) -- 
          (pbase + 4 * ppasx              - 0.5 * ppasyA)
          withpen pencircle scaled 2bp;

drawarrow (pbase + 2 * ppasx - 1 * ppasy + 1 * ppasxA - 0.1 * ppasyA) -- 
          (pbase + 3 * ppasx - 1 * ppasy              - 0.1 * ppasyA)
          withpen pencircle scaled 2bp;
...
\startchapter
  [title=Préface%
  \footnote{Cette   préface    a   commencé   avec   l'intention    d'être   une
    traduction|/|adaptation  à \ConTeXt\  de la  préface de  \quotation{The \TeX
...
également \quotation{beaux}.   A cet égard,  nous pouvons mentionner ici  ce que
...
toujours  très utiles  pour  démarrer  avec \ConTeXt,  celles  contenues le  sur
\from[wiki], qui, au  moment où nous écrivons ces lignes,  est plein remaniement
...
\startSmallPrint
  ...
  Il est vrai  que de ce type d'information exigerait  un fort investissement en
...
Etant donné que ce qui est expliqué dans ce document provient essentiellement de
...
non pas  à \ConTeXt\ mais  à \TeX,  le système de  base mais j'ai  compris qu'il
    ...
    \startitemize
      ...
      \startitemize
        ...
        \item Le \in{Chapitre}[cap:pages]  se concentre sur les  pages~: taille,
          éléments  qui la  composent, conception  ou composition  (c'est-à-dire
